...
/**
 * This class is used to treat Traininng Sceneario Editor mode as a server
 * which can send to and receive information from client (window-based editor)
 * Basically, it is all about server-client programming with c# that can be refered
 * from MSDN of Microsoft, or any other resource from Googling.
 */ 

public class ServerTraining : MonoBehaviour { 
	int counter = 0;
	private Rect grp;
	private Rect box;
	...
	private ExpandColapseEngine script;
	...
	void Awake ()
	{
		...
		script = transform.GetComponent(typeof(ExpandColapseEngine)) as ExpandColapseEngine;
	...
	void Update ()
	{
		...
		if(firstAssign)
		{
			...
			if(currentComponentInEditor != "")
			{
				...
				// Do some things with engine component after construct it with Component name received
				// from client. refer to method of EngineComponent to understand method purpose.
