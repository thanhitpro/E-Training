...
namespace ETraining.UI.StepControls
{
	 ...
	 * This class inherits GUIBase, and is used to control Next Button on the title bar docked in top edge of screen
	 * which moves learning process to next component of engine or transmission
	 */
	public class TittleNext : GUIBase
	{
		void Start ()
		{
			...
			x = Screen.width/2 + (Mathf.CeilToInt((float)Screen.width / (float)standardWidth * 374.0f /2.0f)) ; //(int)((float)Screen.width / (float)standardWidth * 374)
		}
