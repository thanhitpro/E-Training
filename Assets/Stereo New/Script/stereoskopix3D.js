...
enum mode3D {Anaglyph, SideBySide, OverUnder,Interlace, Quit};
private var modeStrings : String[] = ["Anaglyph", "Side By Side","Over Under","Interlace",  "Quit"];
...
public var interlaceRows : int = 954;
...
private var windowRect : Rect = Rect(20,20,200,200);
...
function OnRenderImage (source:RenderTexture, destination:RenderTexture) {
   ...
   switch (format3D) {
	...
	break;
	case mode3D.Quit:
		Application.LoadLevel("Training new");
   	default:
   	break;
   }
   GL.PopMatrix();
...
function OnGUI () {
   	...
   	if (GuiVisible) {
    	windowRect = GUILayout.Window (0, windowRect, DoWindow, "3D Control");
...
function DoWindow (windowID : int) {
	GUILayout.BeginHorizontal();
		GUILayout.BeginVertical();
			...
			format3D = GUILayout.SelectionGrid (format3D, modeStrings,1,GUILayout.MaxWidth(200), GUILayout.MaxHeight(500));
			if (GUI.changed) {
